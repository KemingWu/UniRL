...
|---|---|---|
| `ltx2_t2v_dancegrpo_trainside_lora32_33f_10step_0614d_mycode` | SUBMITTED (0614) | LTX2 T2V DanceGRPO trainside. After a full systematic audit of the package (vs WAN21/SD3 + diffusers source) instead of serial single-bug fixes. Prior `0614c` ENDed at `RolloutTrack.__init__() got an unexpected keyword argument 'group_ids'`. This run fixes that PLUS the remaining latent crashes/correctness bugs found in the audit: VAE decode (timestep_conditioning, [-1,1]→[0,1] rescale, Videos.from_list packing), connector chain (LTX2TextConnectors + stacked-Gemma-layers text conditioning), CFG empty-negative. See change #3c. |
