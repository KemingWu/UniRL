...
|---|---|---|
| `ltx2_t2v_dancegrpo_trainside_lora32_33f_10step_0614c_mycode` | SUBMITTED (0614) | LTX2 T2V DanceGRPO trainside. Verifies the SDE-strategy/segment-storage fix. Prior `0614b` passed the audiovisual-forward fix but ENDed at `AttributeError: 'FlowSDEStrategy' object has no attribute 'denoise_with_logp'`. Fixed: rewrote `generate`/`replay` to the framework SDE contract (`strategy.denoise(...)`, sparse `LatentSegment.{latents,indices}` storage, `latents_at`), mirroring WAN21. Stacked on noise-recipe + audiovisual-forward fixes. |
