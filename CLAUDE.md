...
|---|---|---|
| `ltx2_t2v_dancegrpo_trainside_lora32_33f_10step_0614e_mycode` | SUBMITTED (0614, instance 8b1d81139eb5dfc1019ec9e3f9fc272f, commit `a7847a9`) | LTX2 T2V DanceGRPO trainside. Verifies the dynamic-σ fix for BLURRY videos. Prior `0614d` ran end-to-end and reward rose, but videos were blurry — the engine used a static `shift=1.0` σ schedule (no `build_schedule_policy`) instead of LTX2's dynamic exponential μ=2.05 + shift_terminal=0.1. Fixed (change #10): `LTX2SchedulePolicy` constant-μ exponential shift. Stacked on all prior 0614d fixes. |
...
| `hi3_it2i_dancegrpo_editreward_mimo_fsdp_0614c_2node` | SUBMITTED (0614) | HI3 80B it2i editreward, 2-node (node0=reward, node1=train). Prior `0614b` got past the `model_version` AttributeError but ENDed at rollout with `OSError: Can't load tokenizer for 'PreTrainedTokenizerFast(...)'` — `_ensure_tokenizer_loaded` passed the loaded tokenizer OBJECT to the ckpt's `load_tokenizer`, which calls `from_pretrained(tok)` expecting a PATH. Fixed: pass `pretrained_path` instead (change #8). Also: ENTRY=train_diffusion, 2-node single-train-node pattern. |
