...
| `flux2klein_4b_editreward_0613i_mycode` | RUNNING (⚠ source image NOT conditioned — superseded) | Flux2Klein 4B + EditReward. New: ckpt save (adapter, every 100), input+output image side-by-side in wandb media. Fixed: data paths (jsonl rewritten to absolute). **BUG FOUND: the pipeline was pure T2I — `primitives["image"]` (source) never reached the transformer, so edited images ignored the source. Fixed in commit (see change #9); needs resubmit.** |
